...
for subj=1:28
    ...
    for window=1:window_length:min(length(infant_torso(:,1)),length(infant_ankle(:,1)))
        ...
        for axis_perm=1:6
            ...
            [R_x, P_x]=corrcoef(infant_torso_x,infant_ankle(window:window_end,axis_order(1)));
            [R_y, P_y]=corrcoef(infant_torso_y,infant_ankle(window:window_end,axis_order(2)));
            [R_z, P_z]=corrcoef(infant_torso_z,infant_ankle(window:window_end,axis_order(3)));
            
            %if all not bigger than 0.5, set as zero
            x_correlation(axis_perm)=abs(R_x(2,1))*(abs(R_x(2,1))>=0.5&&abs(R_y(2,1))>=0.5&&abs(R_z(2,1))>=0.5);
            y_correlation(axis_perm)=abs(R_y(2,1))*(abs(R_x(2,1))>=0.5&&abs(R_y(2,1))>=0.5&&abs(R_z(2,1))>=0.5);
            z_correlation(axis_perm)=abs(R_z(2,1))*(abs(R_x(2,1))>=0.5&&abs(R_y(2,1))>=0.5&&abs(R_z(2,1))>=0.5);
        ...
        if(max(sum_of_all_correlations)>0)
            ...
            for random_index=1:100
               ...
               [R_x, P_x]=corrcoef(infant_torso_x,random_window_1(1:length(infant_torso_x)));
               [R_y, P_y]=corrcoef(infant_torso_y,random_window_2(1:length(infant_torso_y)));
               [R_z, P_z]=corrcoef(infant_torso_z,random_window_3(1:length(infant_torso_z)));
