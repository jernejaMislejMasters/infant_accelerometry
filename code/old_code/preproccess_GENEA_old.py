...
x_axis=medfilt(GENEA_csv['x'][15:data_points+15],5)
y_axis=medfilt(GENEA_csv['y'][15:data_points+15],5)
z_axis=medfilt(GENEA_csv['z'][15:data_points+15],5)
#estimate night between 21.00 and 7.00
first_timestamp=GENEA_csv['t'][15]
...
for window in range(0,len(x_axis),window_points):
	...
	if (std_x<=no_wear_std and std_y<=no_wear_std and std_z<=no_wear_std) or (span_x<=no_wear_span and span_y<=no_wear_span and span_z<=no_wear_span):
		...
		if ( abs(line_fit_x[0])<float('1e-07') and  abs(line_fit_y[0])<float('1e-07') and abs(line_fit_z[0])<float('1e-07')):
			...
			no_wear.append(0)
			'''mark too much drift with green
			ploty.figure(1)
			ploty.plot(range(window,window+len(cumulative_array_x)),cumulative_array_x,'g-')
			ploty.draw()
			ploty.figure(2)
			ploty.plot(range(window,window+len(cumulative_array_y)),cumulative_array_y,'g-')
			ploty.draw()
			ploty.figure(3)
			ploty.plot(range(window,window+len(cumulative_array_z)),cumulative_array_z,'g-')
			ploty.draw()
			'''		

		#dont check for baseline drift		
		#no_wear.append(1)
		#ploty.figure(1)
		#ploty.plot(range(window,window+len(cumulative_array_x)),cumulative_array_x,'r-')
		#ploty.draw()
...
#ploty.show(block=False)
#plot filtered wear signal
#filtered_x_axis_wear=medfilt(x_axis_wear,9)
#ploty.figure(3)
#ploty.plot(range(0,len(filtered_x_axis_wear)),filtered_x_axis_wear,'g-')
ploty.figure(1)
...
#####save memory but consume time, read line by line
'''	
with open(sys.argv[1], 'rb') as GENEA_csv:
	row_count=0
	stats_array_count=0
        cumulative_array_x=[0]*window_points
        cumulative_array_y=[0]*window_points
        cumulative_array_z=[0]*window_points

	std_array_x=[0]*stats_nr
	std_array_y=[0]*stats_nr
	std_array_z=[0]*stats_nr

        avg_array_x=[0]*stats_nr
        avg_array_y=[0]*stats_nr
        avg_array_z=[0]*stats_nr

        span_array_x=[0]*stats_nr
        span_array_y=[0]*stats_nr
        span_array_z=[0]*stats_nr

	for row in csv.reader(GENEA_csv, delimiter=','):
		#skip header
		if row_count<100:
			row_count+=1
			continue
		
		if row_count>6912102:#48hX60minX60secX40Hz+100 header		
			break

                #take windows of window_points points
		cumulative_array_x[(row_count-100) % window_points]=float(row[1])
                cumulative_array_y[(row_count-100) % window_points]=float(row[2])
                cumulative_array_z[(row_count-100) % window_points]=float(row[3])
		
		if ((row_count-100) % window_points) == window_points-1:
			no_wear_block=0
			
                        avg_x=sum(cumulative_array_x)/len(cumulative_array_x)
                        avg_y=sum(cumulative_array_y)/len(cumulative_array_y)
                        avg_z=sum(cumulative_array_x)/len(cumulative_array_z)
			...
			#arrays, might to be removed in the future
			std_array_x[stats_array_count]=round((sum([(element-avg_x)**2 for element in cumulative_array_x])/len(cumulative_array_x))**(0.5),5)
                        std_array_y[stats_array_count]=round((sum([(element-avg_y)**2 for element in cumulative_array_y])/len(cumulative_array_y))**(0.5),5)
                        std_array_z[stats_array_count]=round((sum([(element-avg_z)**2 for element in cumulative_array_z])/len(cumulative_array_z))**(0.5),5)
	
                        avg_array_x[stats_array_count]=round(avg_x,5)
                        avg_array_y[stats_array_count]=round(avg_y,5)
                        avg_array_z[stats_array_count]=round(avg_z,5)

			span_array_x[stats_array_count]=abs(min(cumulative_array_x)-max(cumulative_array_x))
                        span_array_y[stats_array_count]=abs(min(cumulative_array_y)-max(cumulative_array_y))
                        span_array_z[stats_array_count]=abs(min(cumulative_array_z)-max(cumulative_array_z))

			#check for conditions of wear time
			if (std_x<=no_wear_std and (std_y<=no_wear_std or std_z<=no_wear_std)) or (std_y<=no_wear_std and (std_x<=no_wear_std or std_z<=no_wear_std)) or (std_z<=no_wear_std and (std_y<=no_wear_std or std_x<=no_wear_std)) :
				no_wear_block=1
			
			if not no_wear_block:
                        	if (span_x<=no_wear_span and (span_y<=no_wear_span or span_z<=no_wear_span)) or (span_y<=no_wear_span and (span_x<=no_wear_span or span_z<=no_wear_span)) or (span_z<=no_wear_span and (span_y<=no_wear_span or span_x<=no_wear_span)) :
                        		no_wear_block=1

			#plot the data with according color for inspection
			if no_wear_block:
				ploty.plot(range((row_count-100)-window_points+1,(row_count-100+1)),cumulative_array_x,'r.',linestyle="-")
			else:	
				ploty.plot(range((row_count-100)-window_points+1,(row_count-100+1)),cumulative_array_x,'b.',linestyle="-")

			ploty.draw()
			#if first show
			#if first_show:
				#set plot to False block
			ploty.show(block=False)
				#first_show=0
			#fig.savefig('plot.png')
			#print numpy.std(cumulative_array_x)
			#print stats_array_x[stats_array_count]
			stats_array_count+=1
	
		row_count+=1

#print sum(std_array_x)/len(std_array_x)
#print "next"
#print sum(avg_array_x)/len(avg_array_x)
#print "next"
#print span_array_x
'''	
